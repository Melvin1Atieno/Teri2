<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        // check if simialr investmen has already been made
        $check_similar = mysqli_query($con, "SELECT * FROM investments WHERE InvestorsUsername = '$username' AND Amount = '$ramount'");
        
        if($check_similar->num_rows < 1){
        ...
        // check if the first twenty have already been merged
        $Merge_check = mysqli_query($con, "SELECT * FROM investments WHERE NoOfMachesFound=1");
        if($Merge_check->num_rows >20){
           //select user to merge
            $query = mysqli_query($con,"SELECT * FROM investments WHERE Merged < 2 AND Paid='Paid' LIMIT 1");
            $Username = "";
            $id = "";
            $Amount = "";
            $musername ="";
